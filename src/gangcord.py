...
PROJECT_DIR = APP_DIR if is_frozen_app() else os.path.dirname(APP_DIR)
USER_DATA_DIR = get_user_data_dir()
LOG_DIR = os.path.join(USER_DATA_DIR, "Logs")
CACHE_DIR = os.path.join(USER_DATA_DIR, "Cache")
TEMP_DIR = os.path.join(tempfile.gettempdir(), APP_DIR_NAME)
...
LEGACY_CONFIG_PATHS = (
    os.path.join(os.path.dirname(USER_DATA_DIR), "Game Media Control", "config.json"),
    os.path.join(APP_DIR, "config.json"),
    os.path.join(PROJECT_DIR, "config.json"),
)
APP_ICON_PATH = resource_path("gangcord.ico")
...
GITHUB_RAW_APP_URL = f"https://raw.githubusercontent.com/{GITHUB_REPO}/main/src/gangcord.py"
GITHUB_DOWNLOAD_URL = f"{GITHUB_REPO_URL}/archive/refs/heads/main.zip"
UPDATE_FILES = (
    os.path.join("src", "gangcord.py"),
    os.path.join("src", "game_watcher.py"),
    "README.md",
    "Gangcord.spec",
...
AUTO_LAUNCH_LABELS = {
    "none": "Do not start automatically",
    "valorant": "Start with Valorant",
    "lol": "Start with League of Legends",
    "both": "Start with Valorant and League of Legends",
}
AUTO_LAUNCH_VALUES = {label: value for value, label in AUTO_LAUNCH_LABELS.items()}
WATCHER_EXE_NAME = "GangcordWatcher.exe"
WATCHER_RUN_NAME = "Gangcord Watcher"
...
def com_initialize():
    ...
    if hr == RPC_E_CHANGED_MODE:
        return False
    check_hresult(hr, "Could not initialize Windows Audio COM.")
    return True
...
def query_interface(interface, iid):
    ...
    check_hresult(hr, "Could not open the audio interface.")
    return result
...
def get_default_audio_session_manager():
    ...
    try:
        ...
        check_hresult(hr, "Could not open the audio devices.")
        ...
        check_hresult(hr, "Could not find the default playback device.")
        ...
        check_hresult(hr, "Could not open the audio session manager.")
        return manager
    finally:
        com_release(device)
...
def enumerate_audio_sessions():
    ...
    try:
        ...
        check_hresult(hr, "Could not list audio sessions.")

        count = ctypes.c_int()
        hr = com_method(enumerator, 3, HRESULT, ctypes.POINTER(ctypes.c_int))(enumerator, ctypes.byref(count))
        check_hresult(hr, "Could not count audio sessions.")
...
def set_audio_session_volume(process_name, volume_percent):
    ...
    if not target:
        raise ValueError("Select a media session first.")
    ...
    try:
        ...
        for index in range(count.value):
            ...
            try:
                ...
                check_hresult(hr, "Could not change the volume.")
                changed += 1
            finally:
                com_release(volume_interface)
                com_release(control2)
...
def load_config():
    ...
    for path in checked_paths:
        ...
        try:
            ...
            LAST_CONFIG_ERROR = f"Settings could not be read: {exc}"
            continue
...
def save_config(data):
    ...
    try:
        if not ensure_directory(USER_DATA_DIR):
            raise OSError(f"Could not create the settings folder: {USER_DATA_DIR}")
        ...
        if temp_path and os.path.exists(temp_path):
            try:
                ...
                pass
        LAST_CONFIG_ERROR = f"Settings could not be saved: {exc}"
        return False
...
def start_watcher_process():
    ...
    if not os.path.exists(watcher_path):
        return False, f"Watcher not found: {watcher_path}"
    try:
        subprocess.Popen(
            [watcher_path],
            cwd=APP_DIR,
        ...
        return False, f"Watcher could not be started: {exc}"
    ...
    try:
        ...
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0, winreg.KEY_SET_VALUE) as key:
            if mode == "none":
                ...
                return True, "Automatic game detection is disabled."
            ...
            return True, "Automatic game detection will start with Windows."
    except Exception as exc:
        return False, f"Automatic startup could not be changed: {exc}"
...
def find_release_exe_download_url(release_data):
    ...
    for name, download_url in exe_assets:
        ...
        if normalized_name.startswith("gangcord-setup-"):
            return download_url
    return ""
...
def fetch_latest_release_info():
    ...
    download_url = exe_download_url if is_frozen_app() else GITHUB_DOWNLOAD_URL
    return {
        "kind": "release",
        "latest": tag_name,
        ...
        "manual_update": bool(is_frozen_app() and not exe_download_url),
        "status": (
            "A new installer is available."
            if exe_download_url and is_version_newer(tag_name, APP_VERSION)
            else "The latest release does not contain a Gangcord setup file."
            if is_frozen_app() and not exe_download_url
...
def fetch_latest_raw_version_info():
    ...
    if available:
        status = "A new version is available on GitHub."
    elif version_parts(remote_version) == version_parts(APP_VERSION):
        status = "You have the latest version."
    else:
        status = "This local version is newer than the GitHub version."

    return {
        "kind": "raw_version",
        "latest": f"v{remote_version}",
        "url": GITHUB_REPO_URL,
        "download_url": f"{GITHUB_REPO_URL}/releases/latest" if is_frozen_app() else GITHUB_DOWNLOAD_URL,
        "available": available,
        "manual_update": bool(is_frozen_app()),
        "status": status,
    }
...
def fetch_latest_api_head_info():
    ...
    if current_commit:
        ...
        status = "A new GitHub version is available." if available else "You have the latest version."
    else:
        available = False
        status = "GitHub is reachable. Source updates use a ZIP archive."
...
def fetch_latest_remote_head_info():
    ...
    current_commit = get_current_commit(short=False)
    if not remote_commit:
        raise RuntimeError("The GitHub HEAD revision could not be read.")
...
def fetch_latest_version_info():
    ...
    return {
        "kind": "manual",
        "latest": "Git not found",
        "url": GITHUB_REPO_URL,
        "download_url": GITHUB_DOWNLOAD_URL,
        "available": False,
        "manual_update": True,
        "status": "The GitHub release page can be opened in your browser.",
    }
...
def download_and_apply_zip_update():
    ...
    with tempfile.TemporaryDirectory(prefix="source-update-", dir=TEMP_DIR) as temp_dir:
        ...
        if not roots:
            raise RuntimeError("The update ZIP does not contain a project directory.")
        source_root = roots[0]
        ...
        if not updated:
            raise RuntimeError("The update ZIP does not contain known application files.")

        return "Updated: " + ", ".join(updated)


def validate_downloaded_installer(setup_path):
    if not os.path.exists(setup_path):
        raise RuntimeError("The new setup file was not downloaded.")
    if os.path.getsize(setup_path) < 1024 * 1024:
        raise RuntimeError("The downloaded setup file is too small or incomplete.")
    with open(setup_path, "rb") as handle:
        if handle.read(2) != b"MZ":
            raise RuntimeError("The downloaded file is not a valid Windows executable.")


def installer_scope_switch():
    local_programs = os.path.join(os.environ.get("LOCALAPPDATA", ""), "Programs", APP_NAME)
    if local_programs and os.path.normcase(APP_DIR).startswith(os.path.normcase(local_programs)):
        return "/CURRENTUSER"
    return "/ALLUSERS"


def write_installer_update_script(script_path, setup_path, current_pid):
    update_dir = os.path.dirname(script_path)
    scope_switch = installer_scope_switch()
    script = f"""$ErrorActionPreference = 'SilentlyContinue'
$setupPath = {powershell_quote(setup_path)}
$updateDirectory = {powershell_quote(update_dir)}
while (Get-Process -Id {int(current_pid)} -ErrorAction SilentlyContinue) {{
    Start-Sleep -Milliseconds 250
}}
$process = Start-Process -FilePath $setupPath -ArgumentList @('{scope_switch}', '/CLOSEAPPLICATIONS') -Wait -PassThru
...
def prepare_installer_update(download_url=None):
    ...
    if not download_url or not download_url.lower().split("?", 1)[0].endswith(".exe"):
        raise RuntimeError("The GitHub release does not contain a Gangcord setup file.")

    ensure_directory(TEMP_DIR)
    update_dir = tempfile.mkdtemp(prefix="update-", dir=TEMP_DIR)
    setup_path = os.path.join(update_dir, f"{APP_NAME}-Setup.exe")
    script_path = os.path.join(update_dir, "finish_update.ps1")

    try:
        download_url_to_file(download_url, setup_path, timeout=120)
        validate_downloaded_installer(setup_path)
        write_installer_update_script(script_path, setup_path, os.getpid())
    except Exception:
    ...
    return {
        "mode": "installer",
        "message": "The update is ready. Gangcord will close and the installer will continue.",
...
def capture_region(left, top, width, height):
    ...
    if not screen_dc:
        win_error("Could not read the screen.")

    mem_dc = gdi32.CreateCompatibleDC(screen_dc)
    if not mem_dc:
        user32.ReleaseDC(None, screen_dc)
        win_error("Could not create a memory device context.")

    bitmap = gdi32.CreateCompatibleBitmap(screen_dc, width, height)
    if not bitmap:
        gdi32.DeleteDC(mem_dc)
        user32.ReleaseDC(None, screen_dc)
        win_error("Could not create the screen bitmap.")
    ...
    try:
        ...
        if rows != height:
            win_error("Could not copy the screen data.")
        return buffer.raw
    finally:
        if old_object:
...
def parse_rgb_input(value):
    ...
    if not compact:
        raise ValueError("Enter a color such as #ff4655 or 255,70,85.")
    ...
    raise ValueError("Color format not recognized. Use #ff4655 or 255,70,85, for example.")
...
class GangcordApp(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title(APP_NAME)
        self.minsize(900, 620)
        start_width = max(900, min(1280, self.winfo_screenwidth() - 80))
        start_height = max(620, min(860, self.winfo_screenheight() - 80))
        self.geometry(f"{start_width}x{start_height}")

        for required_dir in (USER_DATA_DIR, LOG_DIR, CACHE_DIR, TEMP_DIR):
            ensure_directory(required_dir)
        ...
        self.update_status_var = tk.StringVar(value="Updates have not been checked yet.")
        self.current_version_var = tk.StringVar(value=get_current_version_text())
        self.latest_version_var = tk.StringVar(value="-")
        self.update_button_text_var = tk.StringVar(value="Check for updates")
        ...
        self.auto_launch_choice_var = tk.StringVar(
            value=AUTO_LAUNCH_LABELS.get(
                normalize_auto_launch_mode(self.config_data.get("auto_launch_mode")),
                AUTO_LAUNCH_LABELS["none"],
            )
        )
        ...
        self.monitor_var = tk.StringVar(value="Stopped")
        ...
        self.lol_monitor_var = tk.StringVar(value="Stopped")
        self.lol_detected_var = tk.StringVar(value="-")
        self.lol_log_var = tk.StringVar(value="Ready.")
    ...
    def on_auto_launch_selected(self, _event=None):
        ...
        if ok:
            ...
            self.show_warning("Automatic launch", message)
    ...
    def build_toolbar(self, parent):
        ...
        config_text = f"Settings: {CONFIG_PATH}"
        ttk.Label(toolbar, text=config_text, style="Muted.TLabel", wraplength=720).grid(
            row=0,
            column=0,
            columnspan=4,
            sticky="w",
        ...
        ttk.Label(toolbar, text="Automatic launch", style="Muted.TLabel").grid(row=1, column=0, sticky="e", padx=(0, 8))
        ...
        self.auto_launch_combo.grid(row=1, column=1, sticky="ew")
        self.auto_launch_combo.bind("<<ComboboxSelected>>", self.on_auto_launch_selected)

        ttk.Label(toolbar, text="Theme", style="Muted.TLabel").grid(row=1, column=2, sticky="e", padx=(18, 8))
        ...
        self.theme_combo.grid(row=1, column=3, sticky="e")
        self.theme_combo.bind("<<ComboboxSelected>>", self.on_theme_selected)

    def build_hero(self, parent):
        self.hero_canvas = tk.Canvas(parent, height=180, highlightthickness=0, bd=0, bg=self.palette["window"])
    ...
    def resize_hero(self):
        ...
        target_height = max(164, min(210, int(window_height * 0.22)))
    ...
    def draw_hero(self):
        ...
        canvas.create_rectangle(0, 0, 7, height - 6, fill="#ff4655", outline="")
        canvas.create_rectangle(7, height - 11, width - 8, height - 6, fill="#2fb7d6", outline="")
        ...
        canvas.create_text(
            ...
            fill=palette["muted"],
            font=("Segoe UI", 10),
            width=min(640, max(300, width - 470)),
            text=(
                "Automatic media control for Valorant and League of Legends. "
                "Monitor a screen region, detect the game state, and control playback or volume."
            ),
        )

        version_x = max(width - 350, 560)
        canvas.create_text(
            version_x,
            26,
            anchor="nw",
            fill=palette["text"],
            font=("Segoe UI", 10),
            text=f"Installed: {self.current_version_var.get()}",
        )
        canvas.create_text(
            version_x,
            48,
            ...
            fill=palette["text"],
            font=("Segoe UI", 10),
            text=f"Latest: {self.latest_version_var.get()}",
        )
        canvas.create_text(
            version_x,
            82,
            ...
            fill=palette["muted"],
            font=("Segoe UI", 10),
            width=max(180, width - version_x - 34),
    ...
    def build_readme_tab(self, parent):
        ...
        ttk.Label(parent, text="Gangcord Guide", style="PageTitle.TLabel").grid(row=0, column=0, sticky="w", pady=(0, 8))
        ttk.Label(
            parent,
            text=(
                "Configure reliable, game-specific media behavior without interacting with game files, "
                "memory, the Riot Client, or anti-cheat components."
            ),
            style="Muted.TLabel",
            wraplength=860,
            justify="left",
        ...
        sections = (
            (
                "About Gangcord",
                "Gangcord watches only the screen region you select and controls normal Windows media playback or an audio session.",
                (
                    "Valorant detects a selected target color, including the default #ff4655.",
                    "League of Legends detects number-like bright shapes in the selected countdown region.",
                    "Each game keeps its own region, media target, action mode, and volume values.",
                ),
            ),
            (
                "Getting Started",
                "Open the game tab you want to configure, then complete these steps:",
                (
                    "Select a compact screen region that contains only the relevant indicator.",
                    "For Valorant, select the target color or enter it manually.",
                    "Refresh media sessions and select the player you want Gangcord to control.",
                    "Choose playback control or volume-only control, test it, and start monitoring.",
                ),
            ),
            (
                "Supported Games",
                "The current detection profiles are designed for Valorant and League of Legends.",
                (
                    "Valorant process checks: VALORANT-Win64-Shipping.exe and RiotClientServices.exe.",
                    "League process checks: LeagueClient.exe, LeagueClientUx.exe, and League of Legends.exe.",
                ),
            ),
            (
                "Automatic Game Detection",
                "The optional Gangcord watcher runs in your Windows user session and checks process names at a low frequency.",
                (
                    "It opens Gangcord only for the games selected in the Automatic launch setting.",
                    "A Windows mutex prevents the main application and watcher from running more than once.",
                    "The watcher never reads process memory and never modifies game files.",
                ),
            ),
            (
                "Configuration",
                f"Personal settings are stored as UTF-8 JSON at {CONFIG_PATH}.",
                (
                    f"Logs: {LOG_DIR}",
                    f"Cache: {CACHE_DIR}",
                    f"Temporary update files: {TEMP_DIR}",
                ),
            ),
            (
                "Troubleshooting",
                "Use a small, stable region and borderless or windowed fullscreen if exclusive fullscreen produces black captures.",
                (
                    "Increase Stable reads if brief transitions trigger unwanted actions.",
                    "Increase color tolerance only when the Valorant color varies because of display effects.",
                    "Refresh media sessions after opening Spotify, a browser, or another player.",
                ),
            ),
            (
                "Privacy",
                "All detection happens locally. Gangcord captures only the selected screen area and does not upload screenshots.",
                (),
            ),
            (
                "License",
                "Gangcord is free for private, non-commercial use. Commercial use requires prior written permission. See LICENSE.txt for details.",
                (),
            ),
            (
                "Version Information",
                f"Installed version: {get_current_version_text()}. Use Check for updates in the header to compare it with the latest GitHub release.",
                (),
            ),
        )

        for index, (title, body_text, bullets) in enumerate(sections, start=2):
            self.add_readme_section(parent, index, title, body_text, bullets)

    def add_readme_section(self, parent, row, title, body_text, bullets):
        body = self.create_card(parent, row, title)
        ttk.Label(
            body,
            text=body_text,
            style="Card.TLabel",
        ...
        for bullet_row, bullet in enumerate(bullets, start=1):
            ttk.Label(
                body,
                text=f"•  {bullet}",
                style="Muted.TLabel",
                wraplength=835,
                justify="left",
            ).grid(row=bullet_row, column=0, sticky="w", pady=(8 if bullet_row == 1 else 4, 0), padx=(6, 0))
    ...
    def update_button_clicked(self):
        if self.manual_update_url:
            ...
            self.update_status_var.set("GitHub was opened in your browser.")
    ...
    def check_for_updates(self):
        ...
        self.manual_update_url = None
        self.update_download_url = None
        self.update_button_text_var.set("Checking...")
        self.update_status_var.set("Checking GitHub for a newer version...")
    ...
    def apply_update_check_result(self, info, error):
        ...
        if error:
            ...
            self.update_button_text_var.set("Check for updates")
            self.update_status_var.set(f"Update check failed: {error}")
            return
        ...
        if manual_update:
            ...
            self.update_button_text_var.set("Open GitHub")
            self.update_status_var.set(info.get("status", "The GitHub release page can be opened in your browser."))
        elif self.update_available:
            self.update_button_text_var.set("Update now")
            self.update_status_var.set(info.get("status", "A new version is available on GitHub."))
        else:
            self.update_button_text_var.set("Check again")
            self.update_status_var.set(info.get("status", "You have the latest version."))

    def install_update(self):
        if self.update_check_running:
            return
        self.update_check_running = True
        self.update_button_text_var.set("Updating...")
        if is_frozen_app():
            self.update_status_var.set("Downloading the Gangcord installer from GitHub...")
        else:
            self.update_status_var.set("Downloading the source update from GitHub...")
        if self.update_button:
            self.update_button.configure(state="disabled")
    ...
    def install_update_worker(self):
        try:
            output = prepare_installer_update(self.update_download_url)
            self.events.put(("update_install", True, output or {"message": "Update completed."}))
    ...
    def apply_update_install_result(self, success, message):
        ...
        if success:
            ...
            self.update_available = False
            self.current_version_var.set(get_current_version_text())
            if result.get("mode") == "installer" and result.get("script_path"):
                self.update_button_text_var.set("Starting update...")
                self.update_status_var.set("The update is ready. Gangcord will close...")
                self.show_info("Update", result.get("message", "The installer is ready and Gangcord will now close."))
                start_hidden_update_script(result["script_path"])
                self.after(250, self.destroy)
            else:
                self.update_button_text_var.set("Check again")
                self.update_status_var.set("Source update completed. Restart the app.")
                self.show_info("Update", "The source update is complete. Restart Gangcord to use the new version.")
        else:
            self.update_button_text_var.set("Update now" if self.update_available else "Check for updates")
            self.update_status_var.set("Update failed.")
    ...
    def build_valorant_tab(self, parent):
        ...
        self.add_status_row(status, 0, "Region", self.region_var)
        self.add_status_row(status, 1, "Color rule", self.red_settings_var)
        self.add_status_row(status, 2, "Monitoring", self.monitor_var)
        self.add_status_row(status, 3, "Detected", self.detected_var)

        controls = self.create_card(parent, 1, "Setup")
        controls.columnconfigure(0, weight=1)
        controls.columnconfigure(1, weight=1)
        ttk.Button(controls, text="Select region", command=self.select_valorant_region).grid(row=0, column=0, sticky="ew", padx=(0, 8))
        ttk.Button(controls, text="Select color", command=self.select_valorant_color).grid(row=0, column=1, sticky="ew")

        self.build_media_section(parent, 2, "valorant")

        settings = self.create_card(parent, 3, "Detection options")
        ...
        ttk.Button(color_controls, text="Save", command=self.save_manual_valorant_color).grid(row=0, column=2, sticky="e")
        ttk.Label(settings, text="Tolerance").grid(row=1, column=4, sticky="w", pady=(10, 0))
        ttk.Entry(settings, textvariable=self.red_difference_var, width=8).grid(row=1, column=5, sticky="ew", padx=(4, 0), pady=(10, 0))

        ttk.Checkbutton(settings, text="React only while Valorant is in the foreground", variable=self.require_valorant_var).grid(
            row=2, column=0, columnspan=6, sticky="w", pady=(10, 4)
        )
        ttk.Radiobutton(settings, text="Send direct Play/Pause commands", variable=self.command_mode_var, value="direct").grid(
            row=3, column=0, columnspan=3, sticky="w"
        )
        ttk.Radiobutton(settings, text="Fallback: toggle media on every state change", variable=self.command_mode_var, value="toggle").grid(
    ...
    def build_lol_tab(self, parent):
        ...
        status = self.create_card(parent, 0, "Status")
        status.columnconfigure(1, weight=1)
        self.add_status_row(status, 0, "Region", self.lol_region_var)
        self.add_status_row(status, 1, "Number rule", self.lol_number_settings_var)
        self.add_status_row(status, 2, "Monitoring", self.lol_monitor_var)
        self.add_status_row(status, 3, "Detected", self.lol_detected_var)

        controls = self.create_card(parent, 1, "Setup")
        controls.columnconfigure(0, weight=1)
        ttk.Button(controls, text="Select region", command=self.select_lol_region).grid(row=0, column=0, sticky="ew")
        ...
        ttk.Label(settings, text="Bright pixels %").grid(row=0, column=2, sticky="w")
        ...
        ttk.Label(settings, text="Min brightness").grid(row=1, column=0, sticky="w", pady=(10, 0))
        ttk.Entry(settings, textvariable=self.lol_bright_min_var, width=8).grid(row=1, column=1, sticky="ew", padx=(4, 12), pady=(10, 0))
        ttk.Label(settings, text="Digit height").grid(row=1, column=2, sticky="w", pady=(10, 0))
        ttk.Entry(settings, textvariable=self.lol_digit_height_var, width=8).grid(row=1, column=3, sticky="ew", padx=(4, 12), pady=(10, 0))
        ttk.Label(settings, text="Digit area").grid(row=1, column=4, sticky="w", pady=(10, 0))
        ttk.Entry(settings, textvariable=self.lol_digit_area_var, width=8).grid(row=1, column=5, sticky="ew", padx=(4, 0), pady=(10, 0))

        ttk.Label(settings, text="Components").grid(row=2, column=0, sticky="w", pady=(10, 0))
        ttk.Entry(settings, textvariable=self.lol_digit_components_var, width=8).grid(row=2, column=1, sticky="ew", padx=(4, 12), pady=(10, 0))

        ttk.Checkbutton(settings, text="React only while League of Legends is in the foreground", variable=self.require_lol_var).grid(
            row=3, column=0, columnspan=6, sticky="w", pady=(10, 4)
        )
        ttk.Radiobutton(settings, text="Send direct Play/Pause commands", variable=self.lol_command_mode_var, value="direct").grid(
            row=4, column=0, columnspan=3, sticky="w"
        )
        ttk.Radiobutton(settings, text="Fallback: toggle media on every state change", variable=self.lol_command_mode_var, value="toggle").grid(
    ...
    def build_media_section(self, parent, row, game):
        ...
        media = self.create_card(parent, row, "Media control")
        for column in range(6):
            media.columnconfigure(column, weight=1)

        ttk.Label(media, text="Target").grid(row=0, column=0, sticky="w")
        ...
        ttk.Button(media, text="Refresh", command=self.refresh_audio_sessions).grid(row=0, column=5, sticky="ew")
        if is_lol:
            self.lol_target_session_combo = combo
        else:
            self.target_session_combo = combo

        ttk.Radiobutton(media, text="Control Play/Pause", variable=action_var, value="media").grid(
            row=1, column=0, columnspan=3, sticky="w", pady=(10, 0)
        )
        ttk.Radiobutton(media, text="Adjust volume only", variable=action_var, value="volume").grid(
            row=1, column=3, columnspan=3, sticky="w", pady=(10, 0)
        )

        ttk.Label(media, text="Dead volume %").grid(row=2, column=0, sticky="w", pady=(10, 0))
        ttk.Entry(media, textvariable=dead_volume_var, width=8).grid(row=2, column=1, sticky="ew", padx=(4, 12), pady=(10, 0))
        ttk.Label(media, text="Alive volume %").grid(row=2, column=2, sticky="w", pady=(10, 0))
        ttk.Entry(media, textvariable=alive_volume_var, width=8).grid(row=2, column=3, sticky="ew", padx=(4, 12), pady=(10, 0))
        ttk.Button(media, text="Test dead", command=lambda selected_game=game: self.test_volume(selected_game, True)).grid(
            row=2, column=4, sticky="ew", padx=(0, 4), pady=(10, 0)
        )
        ttk.Button(media, text="Test alive", command=lambda selected_game=game: self.test_volume(selected_game, False)).grid(
    ...
    def refresh_audio_sessions(self):
        try:
            ...
            message = f"Could not read media sessions: {exc}"
            self.set_game_status("valorant", log=message)
            self.set_game_status("lol", log=message)
            return
        ...
        message = (
            f"Found {len(self.audio_session_labels)} media session(s)."
            if self.audio_session_labels
            else "No active media session was found."
        )
        self.set_game_status("valorant", log=message)
        self.set_game_status("lol", log=message)
    ...
    def audio_target_selected(self, game):
        ...
        self.config_data[keys["target_process"]] = session["process_name"]
        save_config(self.config_data)
        self.set_game_status(game, log=f"Media target saved: {session['process_name']}")
    ...
    def sync_audio_settings(self, game):
        ...
        dead_volume = self.parse_volume_percent(vars_for_game["dead_volume"].get(), "Dead volume %")
        alive_volume = self.parse_volume_percent(vars_for_game["alive_volume"].get(), "Alive volume %")
        selected_label = vars_for_game["target"].get()
        session = self.audio_choice_map.get(selected_label)
        ...
        if session:
            ...
            raise ValueError("Select a media session first or click Refresh.")
    ...
    def test_volume(self, game, is_dead):
        try:
            ...
            self.show_error("Test volume", str(exc))
            return

        state_text = "dead" if is_dead else "alive"
        if changed:
            self.set_game_status(game, log=f"Test: set {audio_settings['target_process']} to {volume:g}% ({state_text}).")
        else:
            self.set_game_status(game, log=f"No active audio session was found for {audio_settings['target_process']}.")
    ...
    def apply_state_action(self, game, state, command_mode, audio_settings):
        ...
        if audio_settings["action_mode"] == "volume":
            ...
            changed = set_audio_session_volume(audio_settings["target_process"], volume)
            if changed:
                return f"Set {audio_settings['target_process']} to {volume:g}% because the state is now {state_text}."
            return f"No active audio session was found for {audio_settings['target_process']}."

        if command_mode == "toggle":
            send_media_toggle()
            return f"Sent a media toggle because the state is now {state_text}."
        if is_dead:
            send_media_play()
            return f"Sent Play because the state is now {state_text}."

        send_media_pause()
        return f"Sent Pause because the state is now {state_text}."
    ...
    def refresh_labels(self):
        ...
        self.red_settings_var.set(
            f"Dead at {self.config_data.get('red_pixel_percent', 1.0)}% matching pixels using {format_rgb(target_rgb)} "
            f"(tolerance {self.config_data.get('valorant_color_tolerance', VALORANT_COLOR_TOLERANCE)})"
        )
        self.lol_region_var.set(region_to_text(self.config_data.get("lol_region")))
        self.lol_number_settings_var.set(
            f"Dead at {self.config_data.get('lol_bright_pixel_percent', 0.25)}% bright pixels "
            f"and {self.config_data.get('lol_min_digit_components', 1)} digit-like component(s) "
            f"(minimum brightness {self.config_data.get('lol_bright_min_value', 150)})"
        )

    def select_valorant_region(self):
        self.show_info(
            "Select region",
            "After you select OK, you have 3 seconds to make Valorant visible. Then drag over the region to monitor.",
        )
        self.withdraw()
    ...
    def select_valorant_color(self):
        self.show_info(
            "Select color",
            "After you select OK, you have 3 seconds to make Valorant visible. Then click the color that represents the dead state.",
        )
        self.withdraw()
    ...
    def select_lol_region(self):
        self.show_info(
            "Select LoL region",
            "After you select OK, you have 3 seconds to make League of Legends visible. Then drag over the number or countdown region.",
        )
        self.withdraw()
    ...
    def open_region_selector(self, game):
        if game == "lol":
            prompt = "Drag over the LoL number region. Press Esc to cancel."
            RegionSelector(self, lambda region: self.region_selected("lol", region), prompt, "#2fb7d6")
        else:
            prompt = "Drag over the Valorant region to monitor. Press Esc to cancel."
    ...
    def open_valorant_color_selector(self):
        prompt = "Click the Valorant target color. Press Esc to cancel."
    ...
    def region_selected(self, game, region):
        ...
        self.lift()
        if not region:
            self.set_game_status(game, log="Region selection was canceled or the region was too small.")
            return

        if game == "lol":
            self.config_data["lol_region"] = region
            self.set_game_status("lol", log="Region saved. Select Start to detect a number in this region.")
        else:
            self.config_data["region"] = region
            self.set_game_status("valorant", log="Region saved. Select a color separately, then select Start.")
    ...
    def valorant_color_selected(self, point):
        if not point:
            self.deiconify()
            self.lift()
            self.set_game_status("valorant", log="Color selection was canceled.")
            return
    ...
    def apply_valorant_color_selection(self, point):
        try:
            ...
            if not target_rgb:
                message = (
                    f"No dominant red color was found at the selected point. The color remains "
                ...
                save_config(self.config_data)
                message = (
                    f"Saved {format_rgb(target_rgb)} "
                    f"({target_pixels}/{total_pixels} pixels, tolerance 0)."
                )
        except Exception as exc:
            message = f"The color could not be read: {exc}"
    ...
    def save_manual_valorant_color(self):
        try:
            ...
            self.show_error("Save color", str(exc))
            return
        ...
        save_config(self.config_data)
        self.refresh_labels()
        self.set_game_status("valorant", log=f"Saved {format_rgb(target_rgb)} manually with tolerance 0.")
    ...
    def sync_valorant_settings(self):
        try:
            ...
            raise ValueError("Interval, Match %, Tolerance, and Stable reads must be numbers.")
        ...
        if color_tolerance < 0 or color_tolerance > 80:
            raise ValueError("Tolerance must be between 0 and 80.")
        if stable_reads < 1:
            raise ValueError("Stable reads must be at least 1.")
    ...
    def sync_lol_settings(self):
        try:
            ...
            raise ValueError("Interval, brightness, digit values, and Stable reads must be numbers.")

        if interval_ms < 100:
            raise ValueError("Interval must be at least 100 ms.")
        if bright_pixel_percent < 0 or bright_pixel_percent > 100:
            raise ValueError("Bright pixels % must be between 0 and 100.")
        ...
        if min_digit_height < 1:
            raise ValueError("Digit height must be at least 1.")
        if min_digit_area < 1:
            raise ValueError("Digit area must be at least 1.")
    ...
    def active_monitor_name(self):
        ...
        if self.active_monitor == "valorant":
            return "Valorant"
        return "another monitor"

    def start_valorant_monitoring(self):
        if self.monitor_is_running():
            self.show_info("Already running", f"{self.active_monitor_name()} is already running. Stop it first.")
            return
        if not self.config_data.get("region"):
            self.show_warning("Region required", "Select a Valorant region first.")
            return
        try:
            self.sync_valorant_settings()
        except Exception as exc:
            self.show_error("Check options", str(exc))
            return
    ...
    def start_lol_monitoring(self):
        ...
        if not self.config_data.get("lol_region"):
            self.show_warning("Region required", "Select a League of Legends region first.")
            return
        try:
    ...
    def start_monitor_thread(self, game, target):
        ...
        self.set_game_status(
            game,
            monitor="Running",
            log="Monitoring started. The window will minimize and can be reopened from the taskbar.",
        )
        self.after(1200, self.iconify)

    def stop_monitoring(self, game=None):
        if not self.active_monitor:
            self.set_game_status(game or "valorant", log="No monitor is active.")
            return

        active = self.active_monitor
        self.stop_event.set()
        self.set_game_status(active, monitor="Stopping...", log="Monitoring is stopping.")
        ...
        while not self.stop_event.is_set():
            try:
                if require_valorant and not is_valorant_foreground():
                    self.events.put(("status", "valorant", "Waiting for Valorant", "-", "Valorant is not in the foreground."))
                    candidate = None
                    candidate_count = 0
                    time.sleep(interval)
                    continue
                ...
                readable = {
                    "red": "Dead (color detected)",
                    "no_red": "Alive (color not detected)",
                }[state]

                detail = f"{readable}  |  {format_rgb(target_rgb)} {percent:.2f}% ({red_pixels}/{total})"
                self.events.put(("status", "valorant", "Running", detail, ""))
                ...
                self.events.put(("status", "valorant", "Error", "-", str(exc)))
        ...
        while not self.stop_event.is_set():
            try:
                if require_lol and not is_lol_foreground():
                    self.events.put(("status", "lol", "Waiting for LoL", "-", "League of Legends is not in the foreground."))
                    candidate = None
                    candidate_count = 0
                    time.sleep(interval)
                    continue
                ...
                readable = {
                    "number": "Dead (number detected)",
                    "no_number": "Alive (no number detected)",
                }[state]

                detail = f"{readable}  |  Bright {percent:.2f}% ({bright_pixels}/{total}), shapes {components}"
                self.events.put(("status", "lol", "Running", detail, ""))
                ...
                self.events.put(("status", "lol", "Error", "-", str(exc)))
                time.sleep(max(interval, 1.0))
        ...
        try:
            while True:
                ...
                if event[0] == "status":
                    ...
                    self.set_game_status(event[1], monitor="Stopped", detected="-", log="Monitoring stopped.")
